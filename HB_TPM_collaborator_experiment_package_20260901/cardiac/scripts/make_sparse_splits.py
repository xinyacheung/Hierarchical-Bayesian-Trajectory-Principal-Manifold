...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    missing_columns = sorted({"patient_id", "n_frames"} - set(manifest.columns))
    if missing_columns:
        raise ValueError(f"manifest missing columns: {missing_columns}")
    if manifest.empty or manifest["patient_id"].duplicated().any():
        raise ValueError("manifest must contain unique patient_id rows")
    n_folds = int(config["n_folds"])
    if n_folds < 3 or len(manifest) < n_folds:
        raise ValueError("n_folds must be >=3 and no larger than patient count")
    strategies = [str(value) for value in config["sampling_strategies"]]
    unknown_strategies = sorted(set(strategies) - {"uniform", "random", "clustered"})
    if unknown_strategies:
        raise ValueError(f"unknown sampling strategies: {unknown_strategies}")
    for strategy in strategies:
        key = f"{strategy}_replicates"
        if key not in config or int(config[key]) < 1:
            raise ValueError(f"config must define positive {key}")
    minimum_frames = int(pd.to_numeric(manifest["n_frames"]).min())
    invalid_budgets = [
        int(k)
        for k in config["shot_budgets"]
        if int(k) < 2 or int(k) > minimum_frames
    ]
    if invalid_budgets:
        raise ValueError(
            f"shot budgets must lie in [2,{minimum_frames}]: {invalid_budgets}"
        )
