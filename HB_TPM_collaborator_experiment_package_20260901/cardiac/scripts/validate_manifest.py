...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    for row in frame.itertuples(index=False):
        ...
        if args.check_arrays:
            ...
            image_shape = time_first_shape(image, int(row.time_axis))
            mask_shape = time_first_shape(mask, int(row.time_axis))
            if len(image_shape) != 3 or len(mask_shape) != 3:
                raise ValueError(
                    f"{row.patient_id}: expected squeezed (T,H,W) arrays; "
                    f"image={image_shape}, mask={mask_shape}"
                )
            if image_shape != mask_shape:
                raise ValueError(
                    f"{row.patient_id}: image/mask shapes differ after axis "
                    f"normalization: {image_shape} vs {mask_shape}"
                )
            if not np.isfinite(np.asarray(image)).all():
                raise ValueError(f"{row.patient_id}: image sequence contains NaN/Inf")
    frame_counts = pd.to_numeric(frame["n_frames"]).to_numpy()
    print(
    ...
    print(
        "Frame counts: "
        f"min={int(frame_counts.min())}, "
        f"median={float(np.median(frame_counts)):.1f}, "
        f"max={int(frame_counts.max())}."
    )
